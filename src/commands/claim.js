                  ...
                  if (!err) {
                    if (result[0].wallet_name == null) {
                      ...
                      switch (timediff) {
                        case 0:
                          const cool = dayjs();
                          cool.set('date', cool.add(1, 'day')).set('hour', 0).set('minute', 0).set('second', 0);
                          claimbox.setAuthor({ name: `${process.env.BOT_NAME} Faucet`, iconURL: process.env.FAIL }).setTitle(`Don't be Greedy!`).setDescription(`You have claimed already. Try again in <t:${cool.unix()}:R>`).setColor(0xff0000);
                          await embed.followUp({ embeds: [claimbox] });
                          break;
                        default:
                          var lost = [];
